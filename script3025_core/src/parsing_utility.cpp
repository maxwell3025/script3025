...
std::vector<AnnotatedToken> tokenize(std::string text) {
  static std::shared_ptr<spdlog::logger> logger =
      ([&] () -> std::shared_ptr<spdlog::logger> {
        logger = spdlog::stderr_color_mt("script3025::tokenize", spdlog::color_mode::always);
  ...
  while (i < text.size()) {
    ...
    if (text.substr(i, 3) == "def" && !(text.length() > i + 3 && is_alphanumeric(text[i + 3]))) {
      i += 3;
      new_token.text = text.substr(start, i - start);
    ...
    if (text.substr(i, 2) == "in" && !(text.length() > i + 2 && is_alphanumeric(text[i + 2]))) {
      i += 2;
      new_token.text = text.substr(start, i - start);
...
ParsedCode parse(std::string text) {
  ...
  std::transform(output.annotated_tokens.begin(), output.annotated_tokens.end(),
                 token_list.begin(), [] (AnnotatedToken d) {
                   return d.token;
                 });
