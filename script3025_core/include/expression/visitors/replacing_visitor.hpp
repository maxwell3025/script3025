...
class ReplacingVisitor : public CloningVisitor {
 public:
  // Construct a replacing visitor that replaces id expressions whose source
  // pointer and id string match. # Parameters
  // - `search_source`: the pointer that id expressions must match in order to
  // be replaced.
  // - `search_id`: the id string that id expressions must match in order to be
  // replaced.
  // - `replacement`: the expression that matching id expressions will be
  // replaced with.
