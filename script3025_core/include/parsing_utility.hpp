...
// @brief Concrete syntax tree for script3025 programs with leaf node <->
// substring mappings.
// @attribute `cst` - The concrete syntx tree of the program.
// @attribute `annotated_tokens` - A vector of `AnnotatedToken`s representing
// the leaf node <-> substring mapping.
struct ParsedCode {
...
// @returns A vector of annotated tokens with the parsed tokens and the
// corresponding text. Note that this method erases whitespace.
[[nodiscard]] std::vector<AnnotatedToken> text_to_tokens(
